            ...
            return(
                <Link key={ind} to={`/cars/version-${ind+1}`}>
                    <div className='card h-md-150 h-200 w-md-200 w-300 mx-10 my-10 d-flex p-md-10' style={{ border:'none', borderRadius: 4, letterSpacing:'0.2rem' }} >
                        <img src={item.images[0]} alt={item.version} className='position-absolute z-0' style={{ left:'0', border: 'none', borderRadius: 4, top:'0', width:'100%', height:'100%', filter:'brightness(50%)'}} />
                        ...
                        <div className={`text-white font-fira-sans font-size-24 z-20 position-relative text-center ${(hover && index === (item.version)) ? 'm-auto' : 'mt-auto mx-auto'}`} style={{ pointerEvents:'none' }}>
