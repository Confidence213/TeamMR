...
const AchievementCard = (props) => {
    ...
    return(
        <div data-aos='fade-down' className='text-white d-flex flex-column align-items-center w-300 m-20 h-auto' >
                    ...
                    props.achieveList.map(item => {
                        return (
                            <div className='row align-items-start my-5'>
                                <div className='col-3 font-fira-sans text-center font-size-12 p-5' style={{ marginRight: '5px', color: '#D25D2A', border: '#D25D2A solid 2px', borderRadius: '10px'}}>{item.position}</div>
                                <div className='col-8 font-fira-sans font-size-20 font-weight-lighter'>{item.description}</div>
                            </div>
                            )
                    })
                }
